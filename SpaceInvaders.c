...
unsigned long prevFired;
unsigned long Fired;

unsigned long GameOver;
...
// ******************* Defining State and enemies *******************
struct State {
	unsigned long x;							// x coordinate
	signed long x_dir;						// -1=moving left, 1=moving right
	unsigned long y;							// y coordinate
	const unsigned char *image[2];		// ptr to image
	long life;										// 0=dead, 1=alive
	unsigned long frames_since_death; 
};
...
void State_Init(void){ int i;
	Player.x = 10;
	Player.y = 47;
	Player.image[0] = PlayerShip0;
	Player.image[1] = PlayerShip0;
	Player.life = 1;
	Player.frames_since_death = 0;
...
void Fire_Missile(void){
	...
	Missile.y = Player.y - 5;
	Missile.image[0] = Missile0;
	Missile.image[1] = Missile1;
	Missile.life = 1;
	Missile.frames_since_death = 0;
	Sound_Shoot();
}

void Move_Enemies(void){ int i;
	if(Enemy[3].x >= 67 && Enemy[3].x_dir == 1){
		for(i=0; i<4; i++){
			Enemy[i].x_dir = -1;
			Enemy[i].y += 2;
		}
	} else if(Enemy[0].x <= 0 && Enemy[0].x_dir == -1){
		for(i=0; i<4; i++){
			Enemy[i].x_dir = 1;
			Enemy[i].y += 2;
		}
		...
		for(i=0; i<4; i++){
			Enemy[i].x += Enemy[i].x_dir;
		}
	}
}

unsigned long Convert(unsigned long sample){ unsigned long A, B;
	A = 16;
	B = 2;
...
void Check_Collision(int i){
	if(Enemy[i].life
		 && Missile.x >= Enemy[i].x - 1 && Missile.x <= Enemy[i].x + 10
		 && Missile.y >= Enemy[i].y - 3 && Missile.y <= Enemy[i].y + 8){
...
void Check_Collisions(void){ int i;
	for(i=0; i<4; i++){
		Check_Collision(i);
	}
}
...
void Draw(void){ int i;
  ...
  for(i=0; i<4; i++){
    if(Enemy[i].life > 0 && Enemy[i].frames_since_death < 2){
     Nokia5110_PrintBMP(Enemy[i].x, Enemy[i].y, Enemy[i].image[FrameCount], 0);
    } else if(Enemy[i].life == 0 && Enemy[i].frames_since_death < 2){
			Nokia5110_PrintBMP(Enemy[i].x, Enemy[i].y, Enemy[i].image[FrameCount], 0);
			Enemy[i].frames_since_death += 1;
		}
  }
	...
	if(Missile.life > 0){
		Nokia5110_PrintBMP(Missile.x, Missile.y, Missile.image[FrameCount], 0);
	}
...
void ADC0_Init(void){ volatile unsigned long delay;
	...
	SYSCTL_RCGC0_R |= 0x00010000; 	// 6) Activate clock for ADC0
	delay = SYSCTL_RCGC0_R;					//    Allow time for clock to stabilize
	SYSCTL_RCGC0_R &= ~0x00000300;	// 7) Configure max sampling rate to 125k/s
...
// *************************** Switches module ***************************
// Initialize Port E for Switches on PE0 and PE1
void Switches_Init(void){ volatile unsigned long delay;
	SYSCTL_RCGC2_R |= 0x00000010;   	 // 1) Activate clock for Port E
	delay = SYSCTL_RCGC2_R;         	 //    Allow time for clock to stabilize
	GPIO_PORTE_DIR_R &= ~0x03; 				 // 2) Make PE0 and PE1 input
	GPIO_PORTE_PCTL_R &= 0x00000000;   // 3) GPIO clear bit PCTL  
	GPIO_PORTE_AFSEL_R &= ~0x03;			 // 4) Disable alternate function on PE0 and PE1 
	GPIO_PORTE_DEN_R |= 0x03;					 // 5) Enable digital I/O on PE0 and PE1
	GPIO_PORTE_AMSEL_R &= ~0x03;			 // 6) Disable analog function on PE0 and PE1
}

unsigned long Read_Fire(void){
	return GPIO_PORTE_DATA_R & 0x01;
}

unsigned long Read_SpecialFire(void){
...
void SysTick_Init(unsigned long period){
	...
	NVIC_ST_RELOAD_R = period - 1;	// Reload value for 30 Hz
	NVIC_ST_CURRENT_R = 0;					// Any write to current clears it
	NVIC_ST_CTRL_R = 0x0007;				// Enable with core clock and interrupts
}

long isGameOver(void){ int i;
	for(i=0; i<4; i++){
		if(Enemy[i].life){
			return 0;
		}
...
void SysTick_Handler(void){
	...
	Fired = Read_Fire();
	Move();
	Semaphore = 1;	// Semaphore flag is set to 1 at 30 Hz
...
// *************************** LED module ***************************
// Initialize Port B for LEDs on PB4 and PB5
void LED_Init(void){ volatile unsigned long delay;
  ...
  GPIO_PORTB_DIR_R |= 0x30;          // 4) PB4 and PB5 output  
  GPIO_PORTB_AFSEL_R &= ~0x30;       // 5) no alternate function       
  GPIO_PORTB_DEN_R |= 0x30;          // 7) enable digital pins PB4 and PB5 
}

// *************************** DAC module ***************************
// Initialize 4-bit DAC on PB0-PB3 (PB0 is LSB)
...
/*
main function for testing ADC and slide potentiometer
*/
int main_testADC(void){
...
/*
main function for testing SysTick interrupts
*/
int main_testSysTick(void){
	TExaS_Init(SSI0_Real_Nokia5110_Scope);
	LED_Init();
...
/*
main function for testing Switches - test using logic analyzer
*/
int main_testSwitches(void){
	TExaS_Init(SSI0_Real_Nokia5110_Scope);
	Switches_Init();
  EnableInterrupts();
  while(1){}
...
/*
main function for testing DAC - test that will not affect LEDs PB4 and PB5
*/
int main_testDAC(void){
	TExaS_Init(SSI0_Real_Nokia5110_Scope);
	LED_Init();
	DAC_Init();
  EnableInterrupts();
  DAC_Out(0);
	...
	DAC_Out(17); // GPIO_PORTB_DATA_R should be 1

	GPIO_PORTB_DATA_R = 0x10;
	DAC_Out(2);	 // GPIO_PORTB_DATA_R should be 0x12 - should not affect PB4 and PB5
	while(1){}
}

int main_testSounds(void){
	TExaS_Init(NoLCD_NoScope);  // set system clock to 80 MHz
	DAC_Init();
	Timer2_Init(80000000/11000);  // 11kHz
	Random_Init(1);
...
int main(void){
  ...
  Nokia5110_ClearBuffer();
	...
	Timer2_Init(80000000/11000);  // 11kHz
	SysTick_Init(80000000/30); // initialize SysTick for 30 Hz interrupts
	EnableInterrupts(); // virtual Nokia uses UART0 interrupts
	
	while(1){
		Display_Start();
		Delay100ms(5);
		while(!Read_Fire() && !Read_SpecialFire()){}
...
int main_og(void){ int AnyLife = 1; int i;
  TExaS_Init(NoLCD_NoScope);  // set system clock to 80 MHz
